    ...
    <AppLayout>
        ...
        <div class="mt-6 sm:flex sm:items-center sm:justify-between">
            ...
            <div class="flex items-center mt-4 gap-x-4 sm:mt-0">
                <a v-if="standards.current_page > 1" @click.prevent="changePage(standards.current_page - 1)"
                    :href="standards.prev_page_url"
                ...
                <a v-if="standards.current_page < standards.last_page"
                    @click.prevent="changePage(standards.current_page + 1)" :href="standards.next_page_url"
                    class="flex items-center justify-center w-1/2 px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md sm:w-auto gap-x-2 hover:bg-gray-100">
                    <span>
                        Next
                    </span>
...
async function performSearch() {
    const query = searchQuery.value.trim();
    try {
        const response = await axios.get(`/catalog/search/${query}`)
...
const changePage = async (page) => {
    try {
        const response = await axios.get(`/catalog/search/${searchQuery.value}?page=${page}`);
        standards.value = response.data.standards;
    } catch (error) {
